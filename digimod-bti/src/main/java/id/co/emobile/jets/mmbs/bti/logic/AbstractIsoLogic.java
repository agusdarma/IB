...
public abstract class AbstractIsoLogic {
...
//	private boolean reversal = false;
	...
	public abstract IsoMsg analyzeResponse(String agentId, String response, String seqNo)
			throws JetsException;

//	public abstract String getSysTraceNo() throws JetsException;

	protected abstract void processHostRef(TransactionTO task, IsoMsg rspIsoMsg);

	protected abstract void processSuccessResponse(TransactionTO task,
			IsoMsg rspIsoMsg) throws JetsException;

	protected abstract IsoMsg buildIso(TransactionTO task) throws JetsException;
	
	public void solve(BtiIsoDelegateAgent agent, TransactionTO task)
			throws JetsException, IOException {
		IsoMsg isoMsg = buildIso(task);
	 ...
	 */
	protected IsoMsg processToHost(BtiIsoDelegateAgent agent, IsoMsg isoMsg)
		...
		IsoMsg rspIsoMsg = agent.processToHost(this, isoMsg.toString(), 
	...
	protected void processResponse(TransactionTO task, IsoMsg rspIsoMsg)
			throws JetsException {
		if (rspIsoMsg == null) {
	...
	protected boolean isSuccess(IsoMsg rspIsoMsg) {
		String responseCode = rspIsoMsg.getItem(IsoMsg.RESPONSE_CODE);
