...
import id.co.emobile.jets.mmbs.bti.helper.NetworkInfoConstant;
...
import id.co.emobile.jets.mmbs.bti.logic.AbstractIsoLogic;
import id.co.emobile.jets.mmbs.bti.logic.IsoLogicFactory;
import id.co.emobile.jets.mmbs.bti.logic.NetworkManagementLogic;
...
public class BtiIsoDelegateAgent implements NewTcpNotifier {
	private static final Logger LOG = LoggerFactory.getLogger(BtiIsoDelegateAgent.class);
...
//				ResultCode.BTI_NO_RESPONSE_FROM_HOST.equals(task.getResultCode())) {
	...
	private boolean checkIfNeedReversal(TransactionTO task, AbstractIsoLogic logic) {
...
//		}
	...
	private String getLogicName(AbstractIsoLogic logic) {
		if(logic == null) {
			return null;
		}
	 ...
	 */
	public IsoMsg processToHost(AbstractIsoLogic logic, String request, String seqNo) throws IOException, JetsException {
		int retry = 0;
		while (retry < maxRetry) {
			try {
				sendData(request);
		...
		IsoMsg isoMsg = waitForResponse(logic, seqNo);
		return isoMsg;
	}
	 ...
	 */
	private IsoMsg waitForResponse(AbstractIsoLogic logic, String seqNo) throws IOException, JetsException {
...
//	}
	...
	public void notifyDataReceived(Channel channel, String message) {
		...
		try {
			IsoMsg isoMessage = HostIsoMsgFactory.createHostIsoMsg().parse(message);
