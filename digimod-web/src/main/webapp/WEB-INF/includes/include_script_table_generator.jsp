		<script type="text/javascript">
			...
			function ajaxPost(formParam, FormID, ProgressRequest, divID, URL, messageSubject)
			{
				...
				}).done(function(resultJson) 
				{
					...
					{
						window.location.href='<s:property value="@id.co.emobile.samba.web.data.WebConstants@PATH_LOGIN_EXPIRED"/>';
					}
					
					$(ProgressRequest).hide();
					resultSearch = JSON.parse(resultJson);
					createPanelTop(FormID, divID, resultSearch, ProgressRequest, URL, "1");
					createTable(FormID, divID, resultSearch, ProgressRequest, URL);
					createPanelTop(FormID, divID, resultSearch, ProgressRequest, URL, "2");
					...
					if(messageSubject == "GenerateTable")
					{
						if(resultSearch.totalRow > 0)
						{
							...
							messageSuccess= "<s:text name="l.prefixSearch" />" +resultSearch.totalRow+ " <s:text name="l.postfixSearch"/>";
						}
						else
						{
							//messageSuccess="<s:text name="l.dataNotFound" />,  <s:text name="l.processTook"/> "+ProcessTime+" <s:text name="l.seconds"/>";
							messageSuccess="<s:text name="l.dataNotFound" />";
						}
					}
					else if(messageSubject == "navigateTable")
					{
						messageSuccess = "<s:text name="l.prefixChangePage" />";
					}
					else if(messageSubject == "sortTable")
					{
						messageSuccess = "<s:text name="l.prefixSort" /> " + sortMessage + " " + sortAscDesc;
					}
					else if(messageSubject == "jumpTable")
					{
						messageSuccess = "<s:text name="l.prefixJumpPage" />" + currentPage;
					}
			...
			function createTable(FormID, divID, resultSearch, progressRequest, URL)
			{
				...
				for(var i=0; i < header.length; i++)
				{
					...
					else
					{
						...
						{
							sortMessage=header[i];
							if(resultSearch.sortOrder==0)
							{
								sortAscDesc="Ascending";
				...
				for (var i=0; i < bodyContent.length; i++)
				{
					...
					{						
						...
						if(linkGenerate.length > 0)
						{
							...
							else if(bodyContent[i][body[j]]==='Delete')
							{
								bodyTemp+="<td><a href='#' onClick='deleteData("+linkGenerate.split("=")[1]+");return false;'>"+bodyContent[i][body[j]]+"</a></td>";
							}
							else
							{
								bodyTemp+="<td><a href='./"+fixEscape(linkGenerate)+"'>"+bodyContent[i][body[j]]+"</a></td>";
							}										
						}
						else
						{
			...
			function createPanelTop(FormID, divID, resultSearch, progressRequest, URL, identifier)
			{
				var panelTop = document.getElementById(divID);
				var panelTopID = divID + "NavigateTop" + identifier;
				var tcl="tcl"+identifier;
				var tcc="tcc"+identifier;
				var tcr="tcr"+identifier;
				if(identifier=="1")
				{					
					panelTop.innerHTML = "";
				}
				...
				var searchText="Jump to : " + "<input type='number' id="+textInputId+" class='ShortLine' style='min-width: 240px;' min='1' max='"+resultSearch.totalPage+"'/>"; 
				var buttonSearch="<input type='button' onClick=\"jumpTable('"+FormID+"', '"+textInputId+"','"+progressRequest+"','"+divID+"', "+resultSearch.rowPerPage+", "+resultSearch.totalPage+", '"+URL+"')\" class='btn btn-danger btn-sm' style='margin: 10px; min-width: 60px;' value='GO'>";
			...
			function createPanelBottom(FormID, divID, resultSearch, progressRequest, URL)
			{
				...
				var panelRowModifier="<div class='btn-group' role='group' style='margin: 10px;' aria-label='Paging'>";
				for (var i=0; i < rowArray.length; i++)
				{
					panelRowModifier+="<input type='button' onclick=\"navigateTable('"+FormID+"', 1,'"+progressRequest+"','"+divID+"',"+rowArray[i]+", '"+URL+"')\" class='btn btn-danger btn-small' value="+rowArray[i]+">";
				}
				panelRowModifier+="</div>";
