...
//
//public class CreateIdBillingCheckerAction extends BaseListAction implements ModuleCheckable {
//	private static final long serialVersionUID = 1L;
//	private static final Logger LOG = LoggerFactory.getLogger(CreateIdBillingCheckerAction.class);
//
//	@Autowired
...
//		listHeader = distMoneyService.findDistHeaderByGroupAndStatus(loginVO.getGroupId(), WebConstants.PAJAK_DIST_STATUS_UPLOADED);
//		return INPUT;
//	}
//	
...
//				throw new SambaWebException(SambaWebException.NE_INVALID_SYSLOGNO);
//			}
//			if (header.getStatus() != WebConstants.PAJAK_DIST_STATUS_UPLOADED) {
//				LOG.warn("Transaction {} is not for checker", loginVO.getGroupId(), header);
//				throw new SambaWebException(SambaWebException.NE_INVALID_SYSLOGNO);
//			}
...
//		try {
//			UserDataLoginVO loginVO = (UserDataLoginVO) session.get(LOGIN_KEY);
//			getHeader().setCheckerRemarks(remarks);
//			if ("PROCESS".equalsIgnoreCase(processType))
//				getHeader().setStatus(WebConstants.PAJAK_DIST_STATUS_CHECKED);
//			else if ("REJECT".equalsIgnoreCase(processType))
//				getHeader().setStatus(WebConstants.PAJAK_DIST_STATUS_REJECTED_CHECKER);
//			else {
//				LOG.warn("Unknown type " + processType);
//				throw new SambaWebException(SambaWebException.NE_INVALID_INPUT);
//			}
//			distMoneyService.verifyDistCheckerPajak(loginVO, getHeader(), otp);
//			return SUCCESS;
//		} catch (SambaWebException fae) {
////			LOG.warn("SambaWebException", fae);
//			WebResultVO wrv = handleJsonException(fae);
...
//		return WebModules.MODULE_APPS_OPERATIONAL_CREATE_ID_BILLING_CHECKER;
//	}
//	
//	public String otp() {
//		UserDataLoginVO loginVO = (UserDataLoginVO) session.get(LOGIN_KEY);
