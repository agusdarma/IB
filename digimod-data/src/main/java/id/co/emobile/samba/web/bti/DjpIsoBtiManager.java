...
public class DjpIsoBtiManager {
	private static final Logger LOG = LoggerFactory.getLogger(DjpIsoBtiManager.class);
	...
	private List<DjpBtiIsoDelegateAgent> listAgent;
...
//	private HandleIsoRequestOtherChannel handleIsoReqOtherChannel;
	...
	public void startManager() {
		listAgent = new ArrayList<DjpBtiIsoDelegateAgent>();
		for (String config: listConfig) {
			...
			for (int i=0; i < amount; i++) {
				DjpBtiIsoDelegateAgent agent = new DjpBtiIsoDelegateAgent();
...
//				agent.setHandleIsoReqOtherChannel(handleIsoReqOtherChannel);
		...
		LOG.info("DJP BTI Manager has been initialized with {} agent(s)", listAgent.size());		
	}
...
//		}
//		LOG.info("DJP BTI Manager has been stopped");		
//	}
	public void stopManager() {
		for (DjpBtiIsoDelegateAgent agent: listAgent) {
...
//	@Override
	...
	private boolean tryToAssignTask(TransactionTO task) throws JetsException {
		...
		DjpBtiIsoDelegateAgent agent = listAgent.get(localIdx);
