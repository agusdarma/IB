...
import id.co.emobile.samba.web.iso.logic.SddAbstractIsoLogic;
...
public class SddBtiIsoDelegateAgent implements NewTcpNotifier {
	private static final Logger LOG = LoggerFactory.getLogger(SddBtiIsoDelegateAgent.class);
...
//	private HandleIsoRequestOtherChannel handleIsoReqOtherChannel;
	...
	public boolean sendNetworkManagement(String networkInfo) {
		LOG.debug("{}: sendNetworkManagement Sdd: {}", id, networkInfo);
		if (inProcess.get()) {
		...
		if (networkLogic != null) {
			...
			try {
				...
				if (SddNetworkInfoConstant.ECHO_TEST.equals(networkInfo) && 
				...
				LOG.error(id + ": Exception in sendNetworkManagement Sdd", e);
			} finally {
				inProcess.set(false);
			}
		}
	...
	public boolean doTask(TransactionTO task) {
		...
		SddAbstractIsoLogic logic = null;
		try {
...
//				ResultCode.BTI_NO_RESPONSE_FROM_HOST.equals(task.getResultCode())) {
	...
	private boolean checkIfNeedReversal(TransactionTO task, SddAbstractIsoLogic logic) {
...
//		}
	...
	private String getLogicName(SddAbstractIsoLogic logic) {
		if(logic == null) {
			return null;
		}
	 ...
	 */
	public SddIsoMsg processToHost(SddAbstractIsoLogic logic, String request, String seqNo) throws IOException, JetsException {
		int retry = 0;
		while (retry < maxRetry) {
			try {
				sendData(request);
		...
		SddIsoMsg isoMsg = waitForResponse(logic, seqNo);
		return isoMsg;
	}
	 ...
	 */
	private SddIsoMsg waitForResponse(SddAbstractIsoLogic logic, String seqNo) throws IOException, JetsException {
	...
	public void sendData(String data) throws IOException {
		if (!tcpClient.isConnected()) {
			...
			if (tcpClient.isConnected()) {
				...
				try {
					inProcess.set(false);
					sendNetworkManagement(SddNetworkInfoConstant.SIGN_ON);
				} finally {
					inProcess.set(p);
				}
			}
