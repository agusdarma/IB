...
public abstract class DjpHostBtiLogic extends DjpAbstractIsoLogic {
...
//	private BtiSequenceManager sequenceManager;
	 ...
	 * java.lang.String)
	 */
	public DjpIsoMsg analyzeResponse(String agentId, String response, String seqNo) throws JetsException {
		getLogger().debug(agentId + ": Analyze response, expected seqNo: " + seqNo);
		
		if (response == null)
			return null;
		DjpIsoMsg isoMsg = SultengIsoMsgFactory.createHostIsoMsgDjp().parse(response);
	...
	protected String getResponseCode(DjpIsoMsg rspIsoMsg) {
		if (rspIsoMsg == null) 
	...
	// Logic Section
	protected String getDefaultAccount(DjpBtiIsoDelegateAgent agent, TransactionTO task) throws JetsException, IOException {
		DjpIsoMsg isoMsg = SultengIsoMsgFactory.createList8AccountDjp(this, task);
